...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.title("Disk Speed")
        self.geometry(f"{self.WIDTH}x{self.HEIGHT}")
        self.protocol("WM_DELETE_WINDOW", self.onclose)

        if sys.platform == "darwin":
            self.bind("<Command-q>", self.onclose)
            self.bind("<Command-w>", self.onclose)
            self.createcommand('tk::mac::Quit', self.onclose)
        ...
        self.label_info_1 = customtkinter.CTkLabel(master=self.frame_info,text="Test speed of storage unit.\n",
                                                   height=80,
